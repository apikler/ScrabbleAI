		...
		unless ($tile_on_board) {
			...
			if ($rack) {
				if ($rack->remove($letter)) {
					...
					$tile = Backend::Tile->new($letter);
				}
				else {
					# The letter is not in $tiles, so use a blank tile.
					$rack->remove("*");
					$tile = Backend::Tile->new("*");
...
sub evaluate {
	...
	for my $tiles (@{$self->get_word_tiles()}) {
		...
		my $multiplier = 1;
		for my $tile (@$tiles) {
			my ($i, $j) = @{Backend::Utils::split_coord($tile->get_location())};
...
sub straight_line {
	...
	foreach my $n (0..1) {
		...
		my @coords_i = Backend::Utils::coord_position(\@coords, 0);
		my @coords_j = Backend::Utils::coord_position(\@coords, 1);
		...
		# tile of this move.
		if (Backend::Utils::same_elements(\@coords_j)) {
			my $j = $coords_j[0];
...
sub get_direction {
	...
	if (@locations > 1) {
		...
		my $i0 = Backend::Utils::split_coord($locations[0])->[0];
		my $i1 = Backend::Utils::split_coord($locations[1])->[0];
		...
		my ($i, $j) = @{Backend::Utils::split_coord($locations[0])};
...
sub get_sorted_locations {
	...
	if ($direction eq 'h') {
		...
		return sort { Backend::Utils::split_coord($a)->[0] <=> Backend::Utils::split_coord($b)->[0] } @locations;
	}
	else {
		# Sort by the j coordinate
		return sort { Backend::Utils::split_coord($a)->[1] <=> Backend::Utils::split_coord($b)->[1] } @locations;
	}
}
...
sub get_word_tiles {
	...
	my ($i_min, $j) = @{Backend::Utils::split_coord($locations[0])};
	my $i_max = Backend::Utils::split_coord($locations[-1])->[0];
	...
	for my $location (@locations) {
		my ($i, $j) = @{Backend::Utils::split_coord($location)};
		my @v_word = (
